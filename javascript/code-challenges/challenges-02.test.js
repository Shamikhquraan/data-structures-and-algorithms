...
const raisedToTheThird = (arr) => {
  ...
  let numPower =Math.pow(item, 3);
arrPower.push(numPower);
...
const addOne = (arr) => {
 ...
 const incremArr = arr.map(item=>item+1);
 return incremArr;
...
const addQuestion = (arr) => {
  const incremArr = arr.map(item=>item+'?');
  return incremArr;
 };
...
let forEachArr=[];
arr.forEach((item)=>{
 let numForEach= Math.pow(2,item);
forEachArr.push(numForEach);
...
const mapTwoToThe = (arr) => {
  const incremArr = arr.map(item=>Math.pow(2,item));
  return incremArr;
};
...
const charCode = (arr) => {
  const incremArr = arr.map(item=>item.charCodeAt(-1));
