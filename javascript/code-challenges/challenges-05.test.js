...
const toLastNames = people => {
  let toLastArray=people.map((item)=>{
  let finArray=item.firstName+' '+item.lastName;
  return finArray ;
  })
return toLastArray;
};
...
const reversedString = (str) => {
  //for who reading this code , ENJOOOOOOOY 
 let myArr = str.split("");
  let maxIdx=myArr.length;
  var arrOne = new Array(maxIdx);
  let arrSpilt = myArr.reduce((acc,val,idx)=>{
   arrOne[maxIdx-1]= val;
           maxIdx--;
    return val;
  },'');
  let arrSpiltNew = arrOne.reduce((acc,val,idx)=>{
   acc=acc+val;
    return acc;
  },'');
  return arrSpiltNew;
