...
const screenForNames = (arr) => {
 let x= arr.map((str) => str.match(/^(Mr|Mrs|Ms|Dr|)\..[A-Za-z]+$/g)).filter((ele) => ele).flat();
...
let newOne=arr.map((ele) => ele[0].toUpperCase() + ele.slice(1));
return newOne;
};
...
let newOne= arr.reduce((a, b) => a + (b.mass > 77 ? " - " + b.name : ""), "").slice(3);
return newOne ;
};
...
const sortBy = (property, arr) => {
 ...
 return (arr.sort((a, b) => a[property].localeCompare(b[property])))}else{  
