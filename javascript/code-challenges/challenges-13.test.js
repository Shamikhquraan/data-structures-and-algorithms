...
const longestString = (arr) => {
  let indxPos = arr.map(result => result.length).indexOf(Math.max(...arr.map(result => result.length)));
  return indxPos;
};
...
const findHappiness = (arr) => {
  let newOne = arr.filter(result => result.includes(':)'));
...
const standardizePhoneNumbers = (arr) => {
  let newOne = arr.map(result => result.substring(1, 4) + result.substring(6, 9) + result.substring(10, 14));
  return newOne;};
...
const onlyOddChars = (str) => {
  let newOne = str.split('').reduce((acc, val, idx) => (idx % 2 !== 0) ? acc += val : acc, '');
  return newOne;};
...
const allHappy = (arr) => {
  let newOne = ((arr.filter(result => !(result.includes(':)'))).length > 0) ? false : true);
  return newOne;};
